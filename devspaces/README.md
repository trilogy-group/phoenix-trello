# Phoenix-Trello Development with DF Devspaces
...
It is assumed that you opened a terminal in `devspaces` folder of the repo.
...
```bash
cd ..
devspaces bind trello
```
4. Grab some container info

```bash
devspaces info trello
```
...
```bash
devspaces exec trello
```

6. Download dependencies
```bash
yes | mix deps.get 
yes | mix deps.update --all
...
## Running Trello via Docker-Compose file
